...
namespace Demo
{
	class GroupJoinDemo
	{
		...
		//	// left outer join
		//	// Note, if ?. is ignored, exception occurs, because subpet has no match in the gj.
		//	// so subpet.Name returns null.
		//	var query = from person in people
		//				join pet in pets on person equals pet.Owner into gj
		//				from subpet in gj.DefaultIfEmpty()
		//				select new { OwnerName = person.FirstName, Pets = subpet?.Name??string.Empty };
		//	foreach (var v in query)
		//	{
		//		Console.WriteLine($"{v.OwnerName + ":",-15}{v.Pets}");
		//	}
